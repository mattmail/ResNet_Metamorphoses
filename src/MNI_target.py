...
for image in test_list:
    ...
    model = meta_model(l, source.shape, device, 51, 6., mu, z0).to(device)
    ...
    for i in range(n_iter):
        ...
        if (i + 1) % 500 == 0:
            ...
            plt.axis('off')
            plt.show()
    L2_norm = (((deformations[l] - target_img)**2).sum())
    L2_norm_list.append(L2_norm.detach().cpu().item())
    time_list.append(time()-t)
    s_mask = (deformations[l] != 0)
    deformations[l] = (deformations[l] - deformations[l][s_mask].mean()) / (deformations[l][s_mask].std() + 1e-30)
    pred_seg = seg_model(deformations[l])
